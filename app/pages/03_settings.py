...
def render_settings():
    ...
    with st.form("settings_form"):
        st.info("Portfolio Intelligence Data Tiers")
        
        av_key = st.text_input("AlphaVantage API Key", value=st.session_state.api_keys.get("ALPHA_VANTAGE_API_KEY", ""), type="password", disabled=True, help="Contact info@mudric.com for access")
        fmp_key = st.text_input("Financial Modeling Prep (FMP) API Key", value=st.session_state.api_keys.get("FMP_API_KEY", ""), type="password", disabled=True, help="Contact info@mudric.com for access")
        openbb_key = st.text_input("OpenBB API Key", value=st.session_state.api_keys.get("OPENBB_API_KEY", ""), type="password", disabled=True, help="Contact info@mudric.com for access")
        polygon_key = st.text_input("Massive.com (formerly Polygon.io) API Key", value=st.session_state.api_keys.get("POLYGON_API_KEY", ""), type="password", disabled=True, help="Contact info@mudric.com for access")
        ...
        st.markdown("""
            <div style="background-color: rgba(59, 130, 246, 0.1); padding: 15px; border-radius: 8px; border-left: 5px solid #3B82F6; margin-bottom: 20px;">
                <p style="margin: 0; color: #E8EAED; font-size: 0.9rem;">
                    <b>Data Connector Access is Restricted.</b><br>
                    To configure custom API keys for AlphaVantage, FMP, OpenBB, or Massive.com, please contact the Mudric Lab team for a Professional License at 
                    <a href="mailto:info@mudric.com" style="color: #00D4AA; text-decoration: none; font-weight: 600;">info@mudric.com</a>.
                </p>
            </div>
        """, unsafe_allow_html=True)

        submitted = st.form_submit_button("Request Access / Save")
        
        if submitted:
            st.warning("Manual configuration is disabled in the Community Edition. Please contact support for license activation.")
